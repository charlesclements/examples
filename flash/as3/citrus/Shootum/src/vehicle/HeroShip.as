package vehicle
{
	...
	/**
	 * This is a common, simple, yet solid implementation of a side-scrolling HeroShip. 
	 * The HeroShip can run, jump, get hurt, and kill enemies. It dispatches signals
	 ...
	 * 
	 * Don't store data on the HeroShip object that you will need between two or more levels (such
	 * as current coin count). The HeroShip should be re-created each time a state is created or reset.
	 */	
	public class HeroShip extends Box2DPhysicsObject
	{
		//properties
		/**
		 * This is the rate at which the HeroShip speeds up when you move him left and right. 
		 */
		[Inspectable(defaultValue="1")]
		public var acceleration:Number = 1;
		
		/**
		 * This is the fastest speed that the HeroShip can move left or right. 
		 */
		[Inspectable(defaultValue="8")]
		public var maxVelocity:Number = 8;
		
		/**
		 * This is the initial velocity that the HeroShip will move at when he jumps.
		 */
		[Inspectable(defaultValue="11")]
		public var jumpHeight:Number = 11;
		
		/**
		 * This is the amount of "float" that the HeroShip has when the player holds the jump button while jumping. 
		 */
		[Inspectable(defaultValue="0.3")]
		public var jumpAcceleration:Number = 0.3;
		
		/**
		 * This is the y velocity that the HeroShip must be travelling in order to kill an Enemy.
		 */
		[Inspectable(defaultValue="3")]
		public var killVelocity:Number = 3;
		
		/**
		 * The y velocity that the HeroShip will spring when he kills an enemy. 
		 */
		[Inspectable(defaultValue="8")]
		public var enemySpringHeight:Number = 8;
		
		/**
		 * The y velocity that the HeroShip will spring when he kills an enemy while pressing the jump button. 
		 */
		[Inspectable(defaultValue="9")]
		public var enemySpringJumpHeight:Number = 9;
		
		/**
		 * How long the HeroShip is in hurt mode for. 
		 */
		[Inspectable(defaultValue="1000")]
		public var hurtDuration:Number = 1000;
		
		/**
		 * The amount of kick-back that the HeroShip jumps when he gets hurt. 
		 */
		...
		/**
		 * Determines whether or not the HeroShip's ducking ability is enabled.
		 */
		...
		//events
		/**
		 * Dispatched whenever the HeroShip jumps. 
		 */
		public var onJump:Signal;
		
		/**
		 * Dispatched whenever the HeroShip gives damage to an enemy. 
		 */		
		public var onGiveDamage:Signal;
		
		/**
		 * Dispatched whenever the HeroShip takes damage from an enemy. 
		 */		
		public var onTakeDamage:Signal;
		
		/**
		 * Dispatched whenever the HeroShip's animation changes. 
		 */		
		...
		protected var _playerMovingHeroShip:Boolean = false;
		...
		/**
		 * Creates a new HeroShip object.
		 */		
		public function HeroShip(name:String, params:Object = null)
		{
		...
		/**
		 * Whether or not the player can move and jump with the HeroShip. 
		 */	
		...
		/**
		 * Returns true if the HeroShip is on the ground and can jump. 
		 */		
		...
		/**
		 * The HeroShip uses the enemyClass parameter to know who he can kill (and who can kill him).
		 * Use this setter to to pass in which base class the HeroShip's enemy should be, in String form
		 * or Object notation.
		 * For example, if you want to set the "Enemy" class as your HeroShip's enemy, pass
		...
		/**
		 * This is the amount of friction that the HeroShip will have. Its value is multiplied against the
		...
		override public function update(timeDelta:Number):void
		{
			...
			if (controlsEnabled)
			{
				...
				if (_ce.input.isDoing("up", inputChannel) && !_ducking)
				{
					velocity.Add(getSlopeBasedMoveAngle());
					moveKeyPressed = true;
				}
				
				if (_ce.input.isDoing("down", inputChannel) && !_ducking)
				{
					velocity.Subtract(getSlopeBasedMoveAngle());
					moveKeyPressed = true;
				}
				
				//If player just started moving the HeroShip this tick.
				if (moveKeyPressed && !_playerMovingHeroShip)
				{
					_playerMovingHeroShip = true;
					_fixture.SetFriction(0); //Take away friction so he can accelerate.
				}
					//Player just stopped moving the HeroShip this tick.
				else if (!moveKeyPressed && _playerMovingHeroShip)
				{
					_playerMovingHeroShip = false;
				...
				if (_onGround && _ce.input.justDid("jump", inputChannel) && !_ducking)
				{
					...
					onJump.dispatch();
					_onGround = false; // also removed in the handleEndContact. Useful here if permanent contact e.g. box on HeroShip.
				}
		...
		/**
		 * Hurts the HeroShip, disables his controls for a little bit, and dispatches the onTakeDamage signal. 
		 */		
		public function hurt():void
		{
			...
			//Makes sure that the HeroShip is not frictionless while his control is disabled
			if (_playerMovingHeroShip)
			{
				_playerMovingHeroShip = false;
				_fixture.SetFriction(_friction);
			}
		...
		override public function handlePreSolve(contact:b2Contact, oldManifold:b2Manifold):void {
			...
			var HeroShipTop:Number = y;
			var objectBottom:Number = other.y + (other.height / 2);
			
			if (objectBottom < HeroShipTop)
