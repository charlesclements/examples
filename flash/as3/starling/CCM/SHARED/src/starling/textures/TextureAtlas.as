...
package starling.textures
{
	...
	public class TextureAtlas
	{
		...
		/** Create a texture atlas from a texture or bitmap by parsing the regions from an XML file. */
		public function TextureAtlas(texture:*, atlasXml:XML=null)
		{
			if (texture is Bitmap) {
				mBitmap = texture;
				texture = Texture.fromBitmap(texture);
			}
		...
		/** Retrieves a subbitmap by name. Returns null if it is not found. */
		public function getBitmap(name:String):Bitmap {
			var region:Rectangle = mTextureRegions[name];
			if (region == null) {
				return null;
			} else {
				var b:BitmapData = new BitmapData(region.width, region.height, true, 0);
				mBitmap.x = -100;
				b.draw(mBitmap, new Matrix( 1, 0, 0, 1, - region.x, - region.y ), null, null);
