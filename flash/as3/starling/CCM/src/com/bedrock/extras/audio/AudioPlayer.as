package com.bedrock.framework.plugin.audio
{
	...
	import com.bedrock.framework.plugin.event.AudioEvent;
	...
	public class AudioPlayer extends DispatcherBase
	{
		/*
		...
		*/
		public function play($startTime:Number = 0, $loops:int = 0, $transform:SoundTransform = null):SoundChannel 
		{
			if ( !this._bolPlaying ) {
				...
				this._bolPaused = false;
				this._objPositionTrigger.startInterval();
		...
		public function stop():void
		{
			if ( this._bolPlaying ) {
				...
				this._objPositionTrigger.stopInterval();
		...
		*/
		...
		{
			this.seekWithTime( MathUtil.getPercentageOfValue( $percent, this.duration ) );
		}
